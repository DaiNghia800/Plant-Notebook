...
class MyGardenController extends ChangeNotifier {
  ...
  Future<void> removePlant(String gardenPlantId) async {
    ...
    try {
      final int profileIndex = _plantProfiles.indexWhere((p) => p.plantId == gardenPlantId || p.id == gardenPlantId);
      if (profileIndex != -1) {
  ...
  Future<void> updatePlantHealthStatus(String gardenPlantId, {String? healthStatus}) async {
    final int index = _savedPlants.indexWhere((p) => p.id == gardenPlantId);
    if (index == -1) return;
