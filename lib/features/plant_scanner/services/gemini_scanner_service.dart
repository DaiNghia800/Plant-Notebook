...
import 'package:plant_notebook/features/plant_scanner/models/plant_analysis_result.dart';

class GeminiScannerService {
  static const int _maxAttempts = 3;
  static const Duration _rateLimitCooldown = Duration(seconds: 30);
  static final Map<String, DateTime> _geminiCooldownUntil = <String, DateTime>{};
  ...
  Future<PlantAnalysisResult> analyzePlantImage(File imageFile) async {
    print('[GeminiService] analyzePlantImage called. Keys: ${_geminiApiKeys.length}, Cooldowns: ${_geminiCooldownUntil.length}');
    ...
    for (var attempt = 1; attempt <= _maxAttempts; attempt++) {
      try {
        ...
        final canRetry = attempt < _maxAttempts && _isTransientError(errorMessage);
        if (!canRetry) rethrow;
  ...
  Future<Map<String, dynamic>> _analyzeWithGeminiFallback(File imageFile) async {
    ...
    for (final apiKey in availableKeys) {
      _nextGeminiStartIndex = (_geminiApiKeys.indexOf(apiKey) + 1) % _geminiApiKeys.length;
      ...
      for (final modelName in modelsToTry) {
        try {
          ...
          print('[GeminiService] Error for $modelName (will try next model): $e');
          lastError = e;
          continue;
        }
      }
  ...
  Future<Map<String, dynamic>> _callGemini({
    ...
    try {
      ...
      if (response.statusCode >= 400) {
        print('[GeminiService] ERROR BODY: ${body.substring(0, body.length.clamp(0, 500))}');
        throw Exception('Gemini HTTP ${response.statusCode} [$modelName]: $body');
      }
  ...
  static List<String> _buildGeminiApiKeys() {
    ...
    final extras = dotenv.env.entries
        .where((e) =>
