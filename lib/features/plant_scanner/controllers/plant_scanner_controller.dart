...
import 'package:plant_notebook/features/plant_scanner/services/gemini_scanner_service.dart';
...
class PlantScannerController extends ChangeNotifier {
  ...
  Future<void> onToggleFlash({Function(String)? onError}) async {
    ...
    if (cameras[currentCameraIndex].lensDirection == CameraLensDirection.front) {
  ...
  Future<void> onCapture({
    ...
    try {
      final XFile photo = await controller.takePicture().timeout(
        ...
        onTimeout: () {
          throw CameraException('Timeout', 'Camera mất quá nhiều thời gian để chụp');
        },
      );
  ...
  Future<void> analyzeImage(String imagePath) async {
    ...
    try {
      ...
      final result = await _scannerService.analyzePlantImage(file).timeout(
